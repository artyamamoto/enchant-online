...
var FightScene = Class.create(BaseScene, {
				...
				switch(r) {
					...
					case 0:
						...
						if (player.__hp <= 0) {
							...
							if (alive == false) {
								this.queue.push({"message" : "{0}たちはしんでしまった。".format(configs.player.name)});
								this.queue.push({"callback" : function() {
																	self.popScene();
															}});
							}
	...
	"initEnemies" : function(datas) {
		...
		for(var i=0; i<datas.length;i++) {
			...
			var enemy = data;
			enemy._img = img = resources.get(data.img);
	...
	"_updateLabel" : function() {
		...
		var i = -1;
		[configs.player.name,'アリーナ','クリフト','ブライ'].forEach(function(namestr) {
			i++;
	...
	"_label" : function(str,x,y) {
		var getLabel = Dialog.prototype.getLabel;
		
		// name 
		var label1 = getLabel.call(null, str);		
		label1.x = x;
		label1.y = y;
	...
	"_arrow" : function(width,height,dir) {
		...
		var sf = new Surface(width,height);
		sf.context.fillStyle = 'rgba(255,255,255,1)';
		var pathes = [];
		switch(dir) {
			case "right" :
				pathes.push([0,0]);
	...
	"_initFrame" : function() {
		...
		var getLabelBg = Dialog.prototype.getLabelBg;
