...
MyMap.Map = Class.create(enchant.extendMap.ExMap, {
	"_pos" : function(x,y) {
...
//		var map = new enchant.extendMap.ExMap(16,16);
	...
	"putPlayer" : function(player,pos) {
		if (!pos) {
			pos = this.findEmpty();
		}
		if (pos) {
			player.put(pos.x, pos.y);
		}
		return false;
	},
	"findEmpty" : function() {
		var height = this._data[0].length;
		var width = this._data[0][0].length;
		...
		while(loop-- > 0) {
			...
			x = x * 16 + 8; 
			y = y * 16 + 8;
			if (! this.hitTest(x,y) && ! this.isSea(x,y)) {
				var pos = this._pos(x,y);
				if (pos) {
					////player.x = pos.x;
					//player.y = pos.y;
...
MyMap.Map.getInstance = function(type) {
	if (!MyMap.Map._instances)
		MyMap.Map._instances = {};
	if (! MyMap.Map._instances[type])
		MyMap.Map._instances[type] = new MyMap.Map(type);
	return MyMap.Map._instances[type];
};
